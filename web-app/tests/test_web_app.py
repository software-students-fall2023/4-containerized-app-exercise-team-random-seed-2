...
def test_upload_transcribe_post(client):
    with patch('app.send_file_to_ml_client') as mock_send_file, \
         patch('app.collection.find_one') as mock_find_one:
        mock_send_file.return_value = MagicMock(status_code=200)
        mock_find_one.return_value = {"transcript": "test transcript", "timestamp": datetime.now()}  # Correct usage of datetime
        data = {
            'audiofile': (BytesIO(b"fake audio data"), 'test.wav')
        }
