...
def transcribe_audio(file):
    ...
    try:
        response = speech_to_text.recognize(
            audio=file.stream, content_type="audio/wav"
        ).get_result()
        return response
    except ApiException as e:
        app.logger.error("IBM Watson API error: %s: %s", e.code, e.message)
...
def save_transcription_to_mongodb(transcription, filename):
    ...
    try:
        ...
        if transcription.get("results", []) and transcription["results"][0].get(
            "alternatives", []
        ):
            transcription_text = transcription["results"][0]["alternatives"][0][
                "transcript"
            ]
