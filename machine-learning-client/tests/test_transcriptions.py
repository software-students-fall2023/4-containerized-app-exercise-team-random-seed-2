...
def test_transcribe_audio_success():
    ...
    with patch("app.speech_to_text.recognize") as mock_recognize:
        mock_recognize.return_value.get_result.return_value = {
...
def test_save_transcription_to_mongodb_failure():
    ...
    with patch("app.MongoClient") as mock_mongo_client:
        ...
        mock_mongo_client.return_value.transcription_database.transcriptions = (
            mock_collection
        )
...
def test_transcribe_success(client, monkeypatch):
    ...
    with patch("app.transcribe_audio") as mock_transcribe_audio, patch(
        "app.save_transcription_to_mongodb"
    ) as mock_save_to_mongodb:
        ...
        response = client.post(
            "/transcribe", data={"file": (io.BytesIO(b"fake_audio_data"), "test.wav")}
        )
